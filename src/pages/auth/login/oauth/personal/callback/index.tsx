...
import { IAuthLoginOauthPersonalCallbackPageProps } from "@pages/auth/login/oauth/personal/callback/type";

const AuthLoginOauthPersonalCallbackPage = ({
  provider,
}: IAuthLoginOauthPersonalCallbackPageProps) => {
  ...
  const redirectUri = `${process.env.REACT_APP_WEB_URL}/auth/login/oauth/personal/callback/${provider}`;
    ...
    (async () => {
      ...
      if (!loginOauthResponse.ok) {
        ...
        if (loginOauthResponse.error === "회원가입이 필요합니다.")
          navigate(
            `/auth/register/oauth/personal?registerToken=${loginOauthResponse.registerToken}`
          );
        else navigate(-1);
...
export default AuthLoginOauthPersonalCallbackPage;
